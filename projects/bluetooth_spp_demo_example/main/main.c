...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DATA_IND_EVT:
        ESP_LOGI(SPP_TAG, "Datos recibidos len:%d", param->data_ind.len);
        
        // Imprimir los datos recibidos en Hexadecimal
        ESP_LOG_BUFFER_HEX("DATA_RX", param->data_ind.data, param->data_ind.len);

        // Ejemplo: Si recibes el caracter '1', encender un LED
        // Nota: param->data_ind.data es un puntero a los bytes recibidos
        if (param->data_ind.len > 0) {
            uint8_t primer_byte = param->data_ind.data[0];
            
            if (primer_byte == '1') {
                ESP_LOGI(SPP_TAG, "Recibido '1': Encendiendo LED...");
                // Aquí pondrías tu lógica para encender el LED:
                // gpio_set_level(GPIO_NUM_2, 1);
            } else if (primer_byte == '0') {
                ESP_LOGI(SPP_TAG, "Recibido '0': Apagando LED...");
                // Aquí pondrías tu lógica para apagar el LED:
